...
export default function AdminPastPapersPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const filePath = `${formGrade.replace(/\s+/g, '_')}/${fileName}`
  ...
  const handleDelete = async (paper: PastPaper) => {
    ...
    try {
      ...
      const urlParts = paper.file_url.split('/past-papers/')
      if (urlParts[1]) {
        await supabase.storage.from('past-papers').remove([urlParts[1]])
      }
