...
export default function GradePromotionPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const pQ = firestoreQuery(profilesRef, where("role", "==", "student"), orderBy("gradeClass"))
      const pSnap = await getDocs(pQ)
      const pData = pSnap.docs.map(doc => ({
        id: doc.id,
        fullName: doc.data().fullName || doc.data().full_name,
        email: doc.data().email,
        gradeClass: doc.data().gradeClass || doc.data().grade_class,
        studentId: doc.data().studentId || doc.data().student_id
      })) as StudentProfile[]

      // Fetch valid students
      const validRef = collection(db, "valid_students")
      const vSnap = await getDocs(firestoreQuery(validRef, orderBy("grade")))
  ...
  const promotionPreview = profiles
    .filter(p => p.gradeClass && p.gradeClass !== "Graduated")
    .map(p => ({
      ...p,
      newGrade: nextGrade(p.gradeClass),
    }))

  const gradeStats = GRADE_ORDER.reduce<Record<string, number>>((acc, g) => {
    acc[g] = profiles.filter(p => p.gradeClass === g).length
    return acc
  }, {})
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      for (const p of promotionPreview) {
        try {
          ...
          await updateDoc(docRef, { 
            gradeClass: p.newGrade,
            updatedAt: new Date().toISOString()
          })
          promotedCount++
          if (p.newGrade === "Graduated") graduatedCount++
        } catch (error: any) {
          errors.push(`Profile ${p.fullName}: ${error.message}`)
        }
      }
      ...
      for (const vs of validStudents) {
        ...
        try {
          const docRef = doc(db, "valid_students", vs.id)
          await updateDoc(docRef, { 
            grade: ng === "Graduated" ? null : ng 
          })
        } catch (error: any) {
          errors.push(`Valid student ${vs.student_id}: ${error.message}`)
        }
      }
  ...
  const graduatedCount = profiles.filter(p => p.gradeClass === "Graduated").length
                ...
                <tbody className="divide-y">
                  {promotionPreview.map(s => (
                    <tr key={s.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <td className="py-3 px-5">
                        <Badge variant="outline" className={gradeColor(s.gradeClass)}>
                          {s.gradeClass ?? "—"}
                        </Badge>
                      </td>
