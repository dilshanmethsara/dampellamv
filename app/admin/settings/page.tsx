...
export default function AdminSettingsPage() {
  ...
  const handleExportData = async () => {
    try {
      ...
      const postsSnap = await getDocs(collection(db, 'announcements'))
  ...
  const handleExportProfilesCSV = async () => {
    try {
      ...
      const rows = profiles.map(p => [
        ...
        p.gradeClass || p.grade_class || "",
        p.created_at?.toDate ? p.created_at.toDate().toLocaleDateString() : p.created_at || ""
      ])
