...
export default function AdminValidStudentsPage() {
  ...
  const fetchStudents = async () => {
    setIsLoading(true)
    try {
      const studentsRef = collection(db, 'valid_students')
      let q = query(studentsRef, orderBy('created_at', 'desc'))

      if (gradeFilter !== "all") {
        q = query(studentsRef, where('grade', '==', gradeFilter), orderBy('created_at', 'desc'))
      }

      const querySnapshot = await getDocs(q)
      const data = querySnapshot.docs.map(doc => ({ 
        ...doc.data() as ValidStudent,
        student_id: doc.id 
      }))
      setStudents(data)
    } catch (error) {
  ...
  const handleAddStudent = async () => {
    ...
    try {
      const studentId = newId.trim().toUpperCase()
      const studentData = {
      ...
      await setDoc(doc(db, 'valid_students', studentId), studentData)
  ...
  const handleDelete = async (studentId: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'valid_students', studentId))
