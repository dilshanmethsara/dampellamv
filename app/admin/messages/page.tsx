...
export default function AdminMessagesPage() {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      let q
      if (statusFilter !== "all") {
        q = firestoreQuery(messagesRef, where('status', '==', statusFilter), orderBy('created_at', 'desc'))
      } else {
        q = firestoreQuery(messagesRef, orderBy('created_at', 'desc'))
      }
      ...
      toast.error("Failed to load messages. Make sure you have created the required index.")
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <DialogHeader>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-muted-foreground">
                {selectedMessage && formatDate(selectedMessage.created_at, 'MMMM dd, yyyy @ h:mm a')}
              </span>
            </div>
